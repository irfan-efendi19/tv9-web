...
<head>
    ...
    <style>
        ...
        .day-card { border-left: 4px solid #006747; }
    </style>
</head>
<body class="antialiased pb-20">

    <!-- Navbar -->
    <nav class="fixed top-0 w-full z-50 px-4 sm:px-8" style="background: rgba(0,40,25,0.9); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(255,255,255,0.1);">
        <div class="max-w-7xl mx-auto flex justify-between items-center h-16">
            <div class="flex items-center gap-10">
                ...
                <div class="flex gap-7">
                    ...
                    <a href="{{ route('jadwal') }}" class="text-sm font-semibold text-yellow-400 border-b-2 border-yellow-400 pb-0.5">Jadwal</a>
        ...
        <div class="space-y-12">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                <div class="bg-slate-50 px-8 py-4 border-b border-slate-100 flex justify-between items-center">
                    <h2 class="text-xl font-black text-emerald-800 uppercase tracking-widest">{{ $dayNames[$dayNum] }}</h2>
                    <span class="text-xs font-bold text-slate-400 uppercase bg-white px-3 py-1 rounded-full border border-slate-200">{{ count($dayPrograms) }} Tayangan</span>
                </div>
                <div class="divide-y divide-slate-50">
                    @foreach($dayPrograms as $prog)
                    <div class="px-8 py-6 hover:bg-slate-50 transition-colors flex flex-col md:flex-row md:items-center gap-4 md:gap-10">
                        <div class="w-32 shrink-0">
                            <span class="text-lg font-bold text-slate-900">{{ \Carbon\Carbon::parse($prog->start_time)->format('H:i') }}</span>
                            <span class="text-slate-300 mx-2">—</span>
                            <span class="text-sm font-medium text-slate-400">{{ \Carbon\Carbon::parse($prog->end_time)->format('H:i') }}</span>
                        </div>
                        <div class="flex-1">
                            <div class="text-xs font-black text-emerald-600 uppercase mb-1">{{ $prog->category ?? 'Umum' }}</div>
