...
$defaultDay = $programs->has($currentDay) ? $currentDay : ($programs->keys()->first() ?? 1);
    ...
    $catColors = [
        ...
        'Olahraga' => 'bg-lime-50 border-lime-200 text-lime-700',
                                    ...
                                    <div x-show="activeDay === {{ $dayNum }}" x-transition:enter="transition ease-out duration-300"
                                        ...
                                        <div class="flex items-end justify-between mb-8 pb-4 border-b-2 border-slate-100">
                                            ...
                                            <div class="hidden sm:block bg-slate-50 border border-slate-200 px-6 py-3 rounded-2xl text-right">
                                                <span class="block text-[10px] font-bold uppercase text-slate-400 tracking-widest mb-1">Kapasitas
                                                    Siaran</span>
        ...
        $isLive = ($dayNum == $currentDay) && ($currentTime >= $prog->start_time && $currentTime <= $prog->end_time);
                                ...
                                @empty
                                    <div class="text-center py-32 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 text-slate-400">
