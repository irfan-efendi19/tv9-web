...
<body class="antialiased min-h-screen" x-data="{ open: false, selected: {} }">
    ...
    <div class="relative py-20 overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
            <h1 class="text-4xl md:text-6xl font-extrabold tracking-tight mb-4">Katalog <span
                    class="text-emerald-400">Program</span></h1>
            ...
            @forelse($catalogs as $item)
                <div @click="selected = {{ json_encode($item) }}; open = true"
                    ...
                    <div class="aspect-[2/3] relative">
                        ...
                        @else
                            <div
                                class="w-full h-full bg-slate-700 flex items-center justify-center text-slate-500 font-bold text-center p-4">
                                {{ $item->title }}
                            </div>
                        @endif

                        <div
                            class="absolute inset-0 p-4 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-opacity bg-gradient-to-t from-black via-black/40 to-transparent">
                            <span
                                class="text-[10px] uppercase font-black text-emerald-400 mb-1">{{ $item->category ?? 'Program' }}</span>
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
        ...
        <div @click.away="open = false" x-show="open" x-transition:enter="transition ease-out duration-300 scale-95 opacity-0"
            ...
            <div class="flex flex-col md:flex-row">
                ...
                <div class="w-full md:w-1/2 aspect-[2/3] bg-slate-800">
                    ...
                    <template x-if="!selected.image_url">
                        <div class="w-full h-full flex items-center justify-center text-slate-600 font-bold text-xl" x-text="selected.title">
                        </div>
                    </template>
                </div>
                ...
                <div class="w-full md:w-1/2 p-8 relative">
                    ...
                    <span class="text-xs font-black text-emerald-400 uppercase tracking-widest mb-2 block"
                    ...
                    <p class="text-slate-400 leading-relaxed overflow-y-auto max-h-60 pr-4 custom-scrollbar"
                        x-text="selected.description || 'Deskripsi program belum tersedia.'"></p>

                    <div class="mt-8">
                        <button @click="open = false"
                            class="w-full py-3 bg-emerald-700 hover:bg-emerald-600 text-white font-bold rounded-xl transition-all shadow-lg shadow-emerald-700/20">
                            Tutup
                        </button>
                    </div>
                </div>
